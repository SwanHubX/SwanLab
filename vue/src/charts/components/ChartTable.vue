<template>
  ...
  <div class="w-full" ref="wrapper">
    <!-- 表格 -->
    <div :style="{ width: typeof tableWidth === 'string' ? tableWidth : `${tableWidth}px` }" ref="table">
      <!-- 表头 -->
      ...
      <!-- 表体, 按一行一行来渲染 -->
      <div v-for="dataColumn in data" :key="dataColumn" :class="{ 'hover:bg-higher': resizeIndex === -1 }" class="line">
        <!-- 单元格 -->
        <div
          v-for="(item, index) in column"
          :key="item.key"
        ...
        >
          <!-- 文本格式 -->
          <div class="w-full overflow-hidden">{{ dataColumn[item.key] || '-' }}</div>
        </div>
      </div>
      <!-- 没有数据时，空占位 -->
...
watch(
  () => props.column,
  () => {
    console.log(wrapper.value.style.width)
    // 遍历列的设置
    props.column.forEach((column, index) => {
      console.log(column)
      const default_width = 150
      widths.value[index] = { value: 0 }
      // 获取所有的列宽
      // 如果自己设置了宽度
      if (column.width) {
