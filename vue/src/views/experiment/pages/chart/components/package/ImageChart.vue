<template>
  ...
  <div class="flex flex-col justify-center grow text-dimmer gap-2" v-if="error">
    ...
    <p class="text-center text-xs">
      <!-- 在此处显示错误信息 -->
      {{ $t('experiment.chart.charts.image.error', { type: error['data_class'], tag: source[0] }) }}
    </p>
  </div>
  <!-- 如果图表数据正确 -->
  <template v-else>
    ...
    <div class="flex flex-col justify-center grow text-dimmer gap-2">
      <SLIcon class="mx-auto h-5 w-5" icon="error" />
      <p class="text-center text-xs">{{ $t('experiment.chart.unkonwn', { type: chart.type }) }}</p>
    </div>
    <!-- 放大效果弹窗 -->
