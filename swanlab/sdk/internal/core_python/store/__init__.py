...
class DataStoreWriter:
    ...
    ``skip=True`` 时进入"跳过落盘"模式（对应 ``core.skip_store``，仅 online 模式合法）：
    ...
    def __init__(self, skip: bool = False):
        ...
        self._skip = skip
        # skip 模式下 write() 的次数，即未持久化的 record 数，仅用于 close() 的 debug 统计
        self._skipped_records: int = 0

    def open(self, filename: Union[Path, str]) -> None:
        """创建并初始化文件，文件已存在时抛出 FileExistsError；skip 模式不创建文件。"""
        if self._skip:
            return
