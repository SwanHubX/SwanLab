...
class OpenApi:
    ...
    def get_experiment(
        ...
        return self.experiment.get_experiment(
            username=username if username else self.http.username, projname=project, exp_id=exp_cuid
        )
    ...
    def get_summary(
        ...
        exp = self.service.get_exp_info(username=username, projname=project, expid=exp_cuid)
    ...
    def get_chart(
        ...
        Returns:
            ApiResponse[List]:
                - code (int): HTTP 状态码
                - errmsg (str): 错误信息, 仅在请求有错误时非空
                - data (List): 实验的指标信息字典, 包含用户训练记录的指标
        """
        return self.experiment.get_chart(
            exp_id=exp_cuid,
            chart_index=index,
            key=key
        )
    ...
    def get_exp_summary(self, *args, **kwargs) -> ApiResponse[Dict]:
        ...
        Args:
            args: 传递给 `get_experiment_summary` 的位置参数
            kwargs: 传递给 `get_experiment_summary` 的关键字参数
    ...
    def list_project_exps(self, *args, **kwargs) -> ApiResponse[List[Experiment]]:
        ...
        Args:
            args: 传递给 `list_experiments` 的位置参数
            kwargs: 传递给 `list_experiments` 的关键字参数
