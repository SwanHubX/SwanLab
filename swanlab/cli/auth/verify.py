...
def verify(local: bool):
    ...
    if not nrc_path.exists():
        sdk.pkg.console.info("You are not logged in. Use `swanlab login` to login.")
        sys.exit(1)
    ...
    console.info(
        "You are logged into", Text(sdk.settings.web_host, style="blue"), "as", Text(username, "bold"), sep=" "
    )
