...
from ..base import BaseType, MediaBuffer
...
class Image(BaseType):
    ...
    def __init__(
        ...
        Parameters
        ----------
        data_or_path: (str or numpy.array or PIL.Image or torch.Tensor or matplotlib figure)
        ...
        elif self.is_pytorch_tensor_typename(self.get_full_typename(data_or_path)):
            # 如果输入为pytorch tensor
            try:
                import torchvision
    ...
    def __convert_size(self, size=None):
        """将size转换为PIL图像的size"""
        if size is None:
            return None
    ...
    def get_full_typename(self, o: Any) -> Any:
    ...
    def parse(self):
        # 文件名称
        hash_name = self.get_hash_by_pil(self.image_data)[:16]
