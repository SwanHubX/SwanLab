...
class DataPorter:
    ...
    def open_for_sync(self, run_dir: str, backend: Literal['python', 'go'] = 'python') -> "DataPorter":
        ...
        if backend == 'python':
            self._pool = ThreadPool(progress_callback=progress_callback)
        elif backend == 'go':
            raise NotImplementedError("swanlab-core is not ready yet.")
        else:
    ...
    def synchronize(self):
        ...
        # 2. 上传列信息
        self._publish((UploadType.COLUMN, columns))
        # 3. 上传标量指标
        self._publish((UploadType.SCALAR_METRIC, scalars))
        # 4. 上传媒体指标
        self._publish((UploadType.MEDIA_METRIC, medias))
        # 5. 上传日志
        self._publish((UploadType.LOG, logs))

        # 等待上传完毕（后台线程会通过回调更新进度条）
        self._pool.finish()
