...
class LogPackageSeed:
    ...
    @staticmethod
    def total_metadata(folder_path: str, *args) -> Callable:
        """
        ...
        async def _():
            swanlog.info(f"Upload the whole file, path: {folder_path}")

        return _

    @staticmethod
    def upload_all_metadata(file_path: str, *args) -> Callable:
        """
        将文件的元数据上传
        ...
        async def _():
            swanlog.warning(f"Upload the metadata of file: {file_path}")

        return _

    @staticmethod
    def diff_upload(file_path: str, diff: DirectorySnapshotDiff, *args) -> Callable:
        """
        将文件的差异部分上传
        :param file_path: 文件路径
        :param diff: 文件差异
        ...
        async def _():
            swanlog.debug(f"Upload the diff of file: {file_path}")
