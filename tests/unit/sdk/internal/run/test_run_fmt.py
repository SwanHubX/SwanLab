...
import swanlab.sdk.internal.run.utils_fmt as F

# ─────────────────────────── flatten_dict ────────────────────────────


class TestFlattenDict:
    """测试 F.flatten_dict 函数"""

    def test_empty_dict(self):
        """空字典直接返回空字典"""
        assert F.flatten_dict({}) == {}

    def test_flat_dict_unchanged(self):
        """无嵌套字典不做任何变换"""
        assert F.flatten_dict({"a": 1, "b": "x"}) == {"a": 1, "b": "x"}

    def test_nested_one_level(self):
        """单层嵌套展开"""
        assert F.flatten_dict({"a": {"b": 1}}) == {"a/b": 1}

    def test_nested_deep(self):
        """深层嵌套展开"""
        assert F.flatten_dict({"a": {"b": {"c": 1}}}) == {"a/b/c": 1}

    def test_mixed_flat_and_nested(self):
        """平铺与嵌套混合"""
        result = F.flatten_dict({"x": 0, "a": {"b": 1, "c": 2}})
        assert result == {"x": 0, "a/b": 1, "a/c": 2}

    def test_int_key_converted_to_str(self):
        """整数 key 自动转为字符串"""
        assert F.flatten_dict({1: {2: "v"}}) == {"1/2": "v"}  # type: ignore

    def test_duplicate_key_latter_wins(self):
        """出现同名展开键时，后者覆盖前者，并触发一次警告"""
        # 先展开嵌套得到 a/b=1，再直接赋值 "a/b"=99 → 发生覆盖
        data = {"a": {"b": 1}, "a/b": 99}
        with patch("swanlab.sdk.internal.run.helper.console") as mock_console:
    ...
    def test_returns_parent_dict_when_provided(self):
        ...
        result = F.flatten_dict({"a": 1}, parent_dict=parent)
...
class TestValidateKey:
    """测试 F.validate_key 函数"""

    @pytest.fixture(autouse=True)
    def reset_warned_keys(self):
        """每个测试前清空全局警告缓存，避免用例间相互污染"""
        F._WARNED_KEYS.clear()
        yield
        F._WARNED_KEYS.clear()

    def test_valid_key_unchanged(self):
        """合法 key 原样返回"""
        assert F.validate_key("loss") == "loss"
        assert F.validate_key("train/acc") == "train/acc"
        assert F.validate_key("v1.0_metric") == "v1.0_metric"

    def test_strip_leading_trailing(self):
        """头尾的空白、. 和 / 被剥离"""
        with patch("swanlab.sdk.internal.run.helper.console"):
            assert F.validate_key("  loss  ") == "loss"
            assert F.validate_key("./key/") == "key"
            assert F.validate_key("...key...") == "key"
    ...
    def test_custom_max_len(self):
        """自定义 max_len 生效"""
        with patch("swanlab.sdk.internal.run.helper.console"):
            result = F.validate_key("abcdef", max_len=3)
        assert result == "abc"

    def test_non_string_input_int(self):
        """整数 key 被转为字符串处理"""
        assert F.validate_key(42) == "42"  # type: ignore

    def test_non_string_input_float(self):
        """浮点 key 被转为字符串处理"""
        assert F.validate_key(3.14) == "3.14"  # type: ignore
    ...
    def test_warning_fires_for_different_keys(self):
        """不同非法 key 各自独立触发一次警告"""
        with patch("swanlab.sdk.internal.run.helper.console") as mock_console:
            F.validate_key("bad key1")
            F.validate_key("bad key2")
