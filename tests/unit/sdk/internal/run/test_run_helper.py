...
class TestFlattenDict:
    ...
    def test_mixed_flat_and_nested(self):
        """平铺与嵌套混合"""
        result = flatten_dict({"x": 0, "a": {"b": 1, "c": 2}})
        assert result == {"x": 0, "a/b": 1, "a/c": 2}

    def test_int_key_converted_to_str(self):
        """整数 key 自动转为字符串"""
        assert flatten_dict({1: {2: "v"}}) == {"1/2": "v"}  # type: ignore
    ...
    def test_returns_parent_dict_when_provided(self):
        ...
        result = flatten_dict({"a": 1}, parent_dict=parent)
...
class TestValidateKey:
    ...
    def test_valid_key_unchanged(self):
        """合法 key 原样返回"""
        assert validate_key("loss") == "loss"
        assert validate_key("train/acc") == "train/acc"
        assert validate_key("v1.0_metric") == "v1.0_metric"
    ...
    def test_custom_max_len(self):
        """自定义 max_len 生效"""
        with patch("swanlab.sdk.internal.run.helper.console"):
            result = validate_key("abcdef", max_len=3)
        assert result == "abc"

    def test_non_string_input_int(self):
        """整数 key 被转为字符串处理"""
        assert validate_key(42) == "42"  # type: ignore

    def test_non_string_input_float(self):
        """浮点 key 被转为字符串处理"""
        assert validate_key(3.14) == "3.14"  # type: ignore
    ...
    def test_warning_fires_for_different_keys(self):
        """不同非法 key 各自独立触发一次警告"""
        with patch("swanlab.sdk.internal.run.helper.console") as mock_console:
            validate_key("bad key1")
            validate_key("bad key2")
