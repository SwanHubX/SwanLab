...
def test_object3d_with_caption():
    """测试带caption的点云"""
    points = np.random.rand(100, 3) * 100
    caption = "Test Point Cloud"
    pc = Object3D(points, caption=caption)
...
def test_object3d_category_validation():
    """测试类别验证"""
    # 生成非整数类别
...
def test_object3d_rgb_validation():
    """测试RGB验证"""
    # 生成非整数RGB值
    xyz = np.random.rand(100, 3)
...
def test_object3d_factory_methods():
    """测试工厂方法"""
    xyz = np.random.rand(100, 3)
...
def test_object3d_different_sizes(n_points):
    """测试不同大小的点云"""
...
def test_object3d_from_json_file_errors():
    """测试从JSON文件加载时的错误情况"""
    # 测试文件不存在
